...
export default class Profile_category_edit_view extends Component { 
  constructor(props)
  {
    super(props);
    this.state={
        category_id: this.props.route.category_id,
        user_id: this.props.route.user_id,
        caption: this.props.route.caption,
  ...
  onDone = ()=>{ 
    ...
    Util.updateCategory(this.state.caption, this.state.description, this.state.user_id, this.state.category_id);
    ...
    return (
        <Image style ={styles.container}
          ...
          <ScrollView style={{height: windowHeight/8, backgroundColor: 'rgb(255,255,255)'}}>
               ...
               <TextInput value={this.state.caption} style={[{width: window.width, height: window.height/15,backgroundColor: 'rgb(242,242,242)',borderWidth: 0, fontSize: windowHeight/35}]}  onChangeText={(txt) => { this.setState({caption: txt})}}/>
               <View style={{height:window.height/20}}/>
               <TextInput multiline={true} value={this.state.description} style={[{width: window.width, height: window.height/5,backgroundColor: 'rgb(242,242,242)',borderWidth: 0, fontSize: windowHeight/35}]}  onChangeText={(txt) => { this.setState({description: txt})}}/>
          </ScrollView>
        </Image>
    );
  }
